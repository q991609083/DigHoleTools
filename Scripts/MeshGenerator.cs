...
public class MeshGenerator : MonoBehaviour
{
	...
	public void GenerateMesh(float[,] map, float squareSize)
	{
        ...
        Vector2[] vecs = new Vector2[this.vertices.Count];
        for (int i = 0; i < this.vertices.Count; i++)
        {
            vecs[i] = new Vector2(this.vertices[i].x, this.vertices[i].y);
        }
        mesh.uv = vecs;
        ...
        this.CreateWallMesh();
        //更新网格碰撞器
        //walls.GetComponent<MeshCollider>().sharedMesh = walls.mesh;
